...
export const mongoCollectionPlans: CollectionPlan[] = [
  ...
  {
    name: "artifacts",
    validator: validator([...tenantRequired, "id", "kind", "storagePath", "sha256", "redacted", "expiresAt"], { ...baseProperties, id: { bsonType: "string" }, kind: { bsonType: "string" }, storagePath: { bsonType: "string" }, sha256: { bsonType: "string" }, redacted: { enum: [true] }, expiresAt: { bsonType: "string" } }),
    indexes: [
  ...
  {
    name: "artifact_blobs",
    validator: validator([...tenantRequired, "artifactId", "content"], { ...baseProperties, artifactId: { bsonType: "string" }, content: { bsonType: "binData" } }),
    indexes: [{ key: { organizationId: 1, projectId: 1, artifactId: 1 }, unique: true, name: "tenant_artifact_blob" }],
  },
  {
    name: "audit_events",
...
export class MongoTracecaseStore implements TracecaseStore {
  ...
  async putArtifactContent(artifact: Artifact, content: Uint8Array) {
    ...
    await this.db.collection("artifact_blobs").replaceOne(
      ...
      { organizationId: artifact.organizationId, projectId: artifact.projectId, artifactId: artifact.id, content: new Binary(Buffer.from(content)) },
      { upsert: true },
    );
  }
