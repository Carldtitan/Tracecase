...
const binaryTypes = new Set(["application/pdf"]);

function resolvedMimeType(file: File): string {
  const declared = file.type.toLowerCase();
  if (imageTypes.has(declared) || textTypes.has(declared) || binaryTypes.has(declared)) return declared;
...
export async function POST(request: Request) {
  try {
    ...
    const resolvedType = resolvedMimeType(file);
    if (!imageTypes.has(resolvedType) && !textTypes.has(resolvedType) && !binaryTypes.has(resolvedType)) return json({ error: "unsupported_attachment_type" }, 415);
